...
This skill ships as part of the default skill set. No separate installation is needed.

### Full install

If you are working from the repository checkout, verify the skill is present with:
...
If you are vendoring only this skill outside the full install, copy the reviewed file from the repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed commit rather than running `git pull`.
