...
- After Plan Mode is approved (mandatory — see AGENTS.md)
...
YOU (main agent) = Lead + Executor
  |
  |-- 1. Copy approved plan VERBATIM from .Codex/plans/ to .pipeline/<slug>/plan.md
...
|-----------------|------|
| "Add 8 skills to Codex" | `add-skills` |
...
1. Read the approved plan from `.Codex/plans/`
...
|------|-----------|---------|
| `plan.md` | Lead | **Verbatim copy** of the approved plan from `.Codex/plans/`. Do NOT rewrite, summarize, or modify. The critic must review against the exact plan the user approved. |
