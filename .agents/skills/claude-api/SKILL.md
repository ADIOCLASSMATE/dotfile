...
name: Codex-api
description: >-
  Anthropic Codex API patterns for Python and TypeScript. Covers Messages API, streaming,
  tool use, vision, extended thinking, batches, prompt caching, and Codex Agent SDK.
  Use when building applications with the Codex API or Anthropic SDKs.
  
  TRIGGER when: "Codex API", "Anthropic SDK", "tool use", "streaming", "prompt caching",
  "extended thinking", "batch API", "vision API"
...
```python
import os
model = os.environ.get("ANTHROPIC_DEFAULT_SONNET_MODEL", "Codex-sonnet-4-0")
```

**Important**: All code examples below use hardcoded model strings for readability. In production, replace every `model="Codex-sonnet-4-0"` with `os.environ.get("ANTHROPIC_DEFAULT_SONNET_MODEL", "Codex-sonnet-4-0")` (Python) or `process.env.ANTHROPIC_DEFAULT_SONNET_MODEL || "Codex-sonnet-4-0"` (TypeScript). Never ship hardcoded model IDs.
